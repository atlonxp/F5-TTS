...
from f5_tts.utils.deep_phonemizer.model.utils import get_dedup_tokens, _make_len_mask, _generate_square_subsequent_mask, PositionalEncoding
...
class ForwardTransformer(Model):
    ...
    def forward(self,x) -> torch.Tensor:         # shape: [N, T]
        """
        Forward pass of the model on a data batch.
...
class AutoregressiveTransformer(Model):
    ...
    def forward_export(self,
        ...
        with torch.no_grad():
            ...
            out_indices = start_index.transpose(1,0)
            for i in range(100):
